...
class RBSheet extends Component {
  ...
  render() {
    ...
    return (
      ...
      <Text style={{textAlign:'center'}}>Change Account</Text>
     
    </View>
    <View style={{padding:20}}>

    {
      this.props.accounts.map((data, key) => {
       return <TouchableOpacity
        onPress={()=>this.props.selectedFromAccount(data)}>
        <View style={{marginVertical:10,marginBottom:1,marginBottomColor:'balck'}}>
        <Text>{data.type} - {data.number}</Text>
        <Text>Available Balance - {data.balance}</Text>
      </View>
      </TouchableOpacity>
      })
    }
